...
max_epoch = 200     #训练的epoch大小
max_images = 100    #每个epoch要训练多少个batch_size
...
mylambda = 10   #g_loss + mylambda * cyc_loss
...
class CycleGAN():
    ...
    def input_setup(self):
        ...
        filenames_A = tf.train.match_filenames_once("D:\\tensorflow_datasets\\monet2photo\\trainA\\*.jpg")
        print(filenames_A)
        self.queue_length_A = tf.size(filenames_A)  #图片A的数目
        print(self.queue_length_A)
        filenames_B = tf.train.match_filenames_once("D:\\tensorflow_datasets\\monet2photo\\trainB\\*.jpg")
        ...
        self.image_A = tf.subtract(tf.div(tf.image.resize_images(tf.image.decode_jpeg(image_file_A),[256,256]),127.5),1)    #rgb/127.5-1映射到[-1,1]
        self.image_B = tf.subtract(tf.div(tf.image.resize_images(tf.image.decode_jpeg(image_file_B),[256,256]),127.5),1)    #rgb/127.5-1映射到[-1,1]
    ...
    def input_read(self, sess):
        ...
        for i in range(max_images):
            ...
            #print(img_size*batch_size*img_layer)
            #print(image_tensor.size)
            #源代码image_tensor.sieze() == int 出现错误：TypeError: 'int' object is not callable
            #更正源代码
    ...
    def loss_calc(self):
        ...
        *_summ -> Summary variables for above loss functions'''
        ...
        g_loss_A = cyc_loss*mylambda + disc_loss_B
        g_loss_B = cyc_loss*mylambda + disc_loss_A
    ...
    def train(self):
        ...
        # Initializing the global variables;居然出现了error：变量未初始化
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(sess.run(self.global_step),max_epoch):
                ...
                if(epoch < 100) :
                    ...
                    curr_lr = learning_rate - learning_rate*(epoch-100)/100
